...
import datetime
from src.utils.cli_args import TrainingArguments, DataArguments, ComparisonArguments
data_params = DataArguments()
com_params = ComparisonArguments()
...
def main():
    ...
    runs_dir = TrainingArguments().save_root_dir_name
    if arg.use_vel:
        arg.work_dir = os.path.join(runs_dir, f"comp_infogcn_{arg.fold_num}_{arg.mode}_vel_{timestamp()}")
    else:
        arg.work_dir = os.path.join(runs_dir, f"comp_infogcn_{arg.fold_num}_{arg.mode}_{timestamp()}")
    init_seed(arg.seed)
    # execute process
